...
class Sequential(Module):
    ...
    def backward(self, input, gradOutput):
        ...
        for i in range(n - 1, 0, -1): 
            self.gradInput = self.modules[i].backward(modules[i - 1].output, self.gradInput)
...
class LogSoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        for i in range(len(input)):
            ...
            self.gradInput[i] = np.dot(gradOutput[i], grad)
        return self.gradInput
...
class LeakyReLU(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        r_s = (input > 0).astype(int)
        l_s = (input <= 0).astype(int)
        self.output = input * r_s + self.slope * input * l_s
        return  self.output
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        r_s = (input > 0).astype(int)
        l_s = (input <= 0).astype(int)
        self.gradInput = gradOutput * r_s + gradOutput * self.slope * l_s
        return self.gradInput
